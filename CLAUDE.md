...
- `src/routes/`: file-based routes. `src/routeTree.gen.ts` is generated by the
  ...
  dashboard, friends, groups, personal, budget, notifications and receipt
  viewer pages live under `_app/`.
...
- **Convex functions** get the user from `requireUser(ctx)` (keyed by
  ...
  fix; the UI shows it via `errorMessage()` in `src/lib/errors.ts`. A
  mutation that must also delete a stored file on a user error returns the
  message instead (a throw rolls the delete back; see `receipts.create`).
  Receipts are uploaded first, then attached by the add/update mutation via
  `attachReceipt` (`convex/lib/receipts.ts`), and deleted with their
  expense via `deleteReceipt`. Totals
